 ...
 */
public class NikoHunterOverlay extends OverlayPanel {

    private final Client client;
    private final NikoHunterConfig config;
    private final NikoHunterPlugin plugin;
    private final NikoHunterScript script;
     ...
     */
    @Inject
    public NikoHunterOverlay(Client client, NikoHunterConfig config, NikoHunterPlugin plugin, NikoHunterScript script) {
        super(plugin);
        this.client = client;
        this.config = config;
        this.plugin = plugin;
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        panelComponent.getChildren().add(LineComponent.builder()
                .left("Caught:")
                .right(String.valueOf(NikoHunterScript.MonkeyTailsCaught))
                .leftColor(Color.WHITE)
                .rightColor(Color.GREEN)
                .build());

        // Display number of failed catches
        panelComponent.getChildren().add(LineComponent.builder()
                .left("Failed:")
                .right(String.valueOf(NikoHunterScript.MonkeyTailsFailed))
                .leftColor(Color.WHITE)
